...
func main() {
	...
	logger := lager.NewLogger("helionplat-app")
	if quiet {
	...
	logger.Info("helionplat-app.starting", lager.Data{"ports": ports})
	...
	for _, port := range ports {
		...
		go func(wg *sync.WaitGroup, port string) {
			...
			logger.Info("helionplat-app.up", lager.Data{"port": port})
			err = <-server.Wait()
			if err != nil {
	...
	wg.Wait()
	logger.Info("shutting helion scaling test app")
}
